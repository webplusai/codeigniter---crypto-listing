...
<?
  ...
  ?>
    ...
    <tr style="font-weight:bold;background-color: #5E4FA0;color: #fff"><th><? echo $is_mobile?'Details':'Name';?></th><? if (!$is_mobile) { echo'<th width="250">Category</th><th width="200">End Date</th>'; }?><th>Ends In</th><? if (!$is_mobile) { echo'<th>Ico Rank</th>'; } ?></tr></thead><tbody>
...
<?
  ...
  $rank = $liveico['ProjICORank'];
  $icorank = '<div class="progress" style="position: relative;">
                      <div class="progress-bar" style="width: '.$liveico['ProjICORank'].'%">
                      </div>
                         <span class="rank">'.$liveico['ProjICORank'].'</span>
                    </div>';
...
<? if ($live_icoview==2)
{
  ...
  echo '<tr'.($liveico['ProjSponsored']?' style="font-weight: bold;font-size: 1.1em;color: #34495e;"':'').' onclick="window.location=\''.$url.'\'"><td><table><tr><td width="25" valign="top">'.($liveico['ProjImage']?'<img src="data:image/png;base64,'.$liveico['ProjImage'].'" height="16" width="16">':'').'</td><td><a href="'.$url.'" title="'.$liveico['ProjDesc'].'"'.($is_mobile?' style="font-weight:bold;"':' class="tooltip_new"').'>'.$liveico['EventName'].'</a>'.$sponsoredstar.'</td></tr>'.($is_mobile?'<tr><td></td><td>'.$end.'</td></tr></table></td>':'</table></td><td>'.$liveico['ProjCatName'].'</td><td>'.$end.'</td>').'<td>'.$date_left_from.'</td><td>'.$icorank.'</td></tr>';

}
else
{
?>
<div class="icobox" style="overflow: hidden;color:#34495e; <? echo $is_mobile?'border-radius: 3px;display: block;width:100%;height:80px;margin-bottom: 10px;position: relative;padding: 8px;':'text-align: center;float: left;width:210px;height:160px;position: relative;margin: 0px 15px 15px 0px;padding:0px;'; ?>border: <? echo $liveico['ProjSponsored']?'#F7A61C 2px solid;font-weight:bold':'1px solid'; ?>; "> <a href="<? echo $url?>">   <? if($liveico['ProjSponsored'] && $liveico['ProjDisableRibbon']==0){?> <span class="ribbon" style="color:#fff;">Gold</span><? }?>   
                <div class="c100 p<? echo $rank; ?> small green tooltip_new" style="position: absolute;margin:3px;" title="This project has an ICORank of <? echo $rank; ?>">
                <span><? echo $rank; ?></span>
                <div class="slice">
                  <div class="bar"></div>
                  <div class="fill"></div>
              ...
              <? 
              $count++;
              if ($count==5) { echo '</div><div>'; $count==0;}
